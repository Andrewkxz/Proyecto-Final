 ...
 * Actúa como puente entre las vistas HTML (Thymeleaf) y la clase Inmobiliaria
 * (Lógica y Estructuras de Datos).
 */
@Controller
public class PropTechController {

    // Instancia estática del cerebro del sistema (Mantiene los datos vivos en
    // memoria)
    ...
    /**
     * Renderiza la vista principal del sistema (Dashboard) pasándole las métricas
     * iniciales.
     */
    @GetMapping("/")
    public String dashboard(jakarta.servlet.http.HttpSession session, Model model) {
        ...
        if(usuarioLogueado == null){
            return "redirect:/login"; // Si no hay sesión, redirecciona al login
        }

        // Pasamos los datos del usuario a la vista para esconder/mostrarCosas
        ...
        // 2. Cargar Asesores para los formularios
        ...
        // 3. Cargar Tareas Administrativas (Cola)
        ...
        // 4. Cargar Solicitudes (Cola)
        String sigSolicitud = plataforma.verSiguienteSolicitud();
        model.addAttribute("siguienteSolicitud",
                sigSolicitud != null ? sigSolicitud : "No hay solicitudes pendientes.");

        // 5. Cargar Historial (Pila)
        ...
        // 7. Generar el JSON del Árbol Binario para el script visual de GoJS
        String treeJson = "[]";
    ...
    public String procesarLogin(@RequestParam String username, @RequestParam String password, 
                                jakarta.servlet.http.HttpSession session, RedirectAttributes redirectAttrs) {
        Usuario u = plataforma.buscarUsuarioPorUsername(username);
        
        if (u != null && u.getPassword().equals(password)) {
            // Credenciales correctas: Guardamos al usuario en la sesión de Spring
    ...
    /**
     * Recibe los datos del formulario, enlaza al asesor y registra el Inmueble en
     * el ABB y Tablas Hash.
     */
    ...
    public String eliminarInmueble(@RequestParam String codigo, RedirectAttributes redirectAttrs) {
        ...
        if (exito) {
            redirectAttrs.addFlashAttribute("mensajeExito",
                    "Inmueble " + codigo + " eliminado de todas las estructuras.");
        } else {
    ...
    public String registrarConsulta(@RequestParam String idCliente, @RequestParam String codigoInmueble,
            RedirectAttributes redirectAttrs) {
        // Tu método en Inmobiliaria no retornaba boolean, así que lo validamos manual
        ...
        if (c != null && i != null) {
            ...
            redirectAttrs.addFlashAttribute("mensajeExito",
                    "Consulta registrada. El grafo de relaciones se ha actualizado.");
        } else {
    ...
    public String descartarInmueble(@RequestParam String idCliente, @RequestParam String codigoInmueble,
        ...
        if (exito) {
            redirectAttrs.addFlashAttribute("mensajeExito",
                    "Inmueble " + codigoInmueble + " descartado para el cliente " + idCliente);
        } else {
            redirectAttrs.addFlashAttribute("mensajeError", "Error al descartar: Cliente o inmueble no encontrados.");
        }
        return "redirect:/";
    }
    ...
    public String registrarAsesor(
        ...
        redirectAttrs.addFlashAttribute("mensajeExito",
                "Asesor " + nombre + " registrado correctamente en la Tabla Hash.");
        return "redirect:/";
    }
    ...
    public String atenderVisita(RedirectAttributes redirectAttrs) {
        ...
        if (visitaAtendida != null) {
            redirectAttrs.addFlashAttribute("mensajeExito", "✅ Atendiendo visita VIP (Nivel "
                    + visitaAtendida.getNivelUrgencia() + "): " + visitaAtendida.getCliente().getNombre());
        } else {
    ...
    /**
     * Confirma que la visita fue atendida.
     */
    ...
    public String confirmarVisita(@RequestParam String idVisita, RedirectAttributes redirectAttrs) {
        ...
        if (exito) {
            redirectAttrs.addFlashAttribute("mensajeExito",
                    "¡Excelente! La visita " + idVisita + " ahora está en estado: Confirmada.");
        } else {
            redirectAttrs.addFlashAttribute("mensajeError",
                    "No se pudo confirmar. Verifique el ID o si la visita ya fue procesada.");
        }
        return "redirect:/";
    }
    ...
    public String registrarIntencion(@RequestParam String idCliente, @RequestParam String codigoInmueble,
        ...
        if (exito) {
            redirectAttrs.addFlashAttribute("mensajeExito",
                    "Intención de negocio registrada para el inmueble " + codigoInmueble);
        } else {
            redirectAttrs.addFlashAttribute("mensajeError", "Error: Cliente o inmueble no encontrado.");
        }
        return "redirect:/";
    }
    ...
    public String verPerfilCliente(@RequestParam String idCliente, RedirectAttributes redirectAttrs) {
        ...
        for (int i = 0; i < cliente.getHistorialConsultas().getSize(); i++)
            consultas.add("🔍 " + cliente.getHistorialConsultas().getData(i).getCodigo());

        List<String> visitas = new ArrayList<>();
        for (int i = 0; i < cliente.getInmueblesVisitados().getSize(); i++)
            visitas.add("🚶 " + cliente.getInmueblesVisitados().getData(i).getCodigo());

        List<String> intenciones = new ArrayList<>();
        for (int i = 0; i < cliente.getIntenciones().getSize(); i++)
            intenciones.add("🎯 " + cliente.getIntenciones().getData(i).getCodigo());

        List<String> negociados = new ArrayList<>();
        for (int i = 0; i < cliente.getInmueblesNegociados().getSize(); i++)
            negociados.add("🤝 " + cliente.getInmueblesNegociados().getData(i).getCodigo());

        // NUEVO: Extracción de descartados
        List<String> descartados = new ArrayList<>();
        for (int i = 0; i < cliente.getInmueblesDescartados().getSize(); i++)
            descartados.add("🚫 " + cliente.getInmueblesDescartados().getData(i).getCodigo());
        ...
        redirectAttrs.addFlashAttribute("listaDescartados", descartados); // Se envía al HTML
    ...
    public String agregarSolicitud(@RequestParam String solicitud, RedirectAttributes redirectAttrs) {
        ...
        redirectAttrs.addFlashAttribute("mensajeExito", "Solicitud de atención encolada: " + solicitud);
        return "redirect:/";
    }

    @PostMapping("/atender-solicitud")
    public String atenderSolicitud(RedirectAttributes redirectAttrs) {
        String resuelta = plataforma.atenderSiguienteSolicitud();
        if (resuelta != null) {
            redirectAttrs.addFlashAttribute("mensajeExito", "✅ Solicitud de cliente atendida: " + resuelta);
        } else {
            redirectAttrs.addFlashAttribute("mensajeInfo", "La cola de solicitudes está vacía.");
        }
        return "redirect:/";
    }
    ...
    public String consultarCiudad(@RequestParam String ciudad, RedirectAttributes redirectAttrs) {
        ...
        if (inmuebles.isEmpty()) {
            ...
            for (int i = 0; i < inmuebles.getSize(); i++) {
                ...
                listaAgrupada.add("🏢 " + inm.getCodigo() + " - " + inm.getBarrioZona() + " ($"
    ...
    /**
     * Ejecuta una búsqueda óptima podando ramas del Árbol Binario según el rango de
     * precio.
     */
    ...
    public String buscarInmuebles(
        ...
        LinkedSimpleList<Inmueble> resultadosPropios = plataforma.buscarInmuebleConFiltros(precioMin, precioMax, zona,
                minHabitaciones);
        ...
        for (int i = 0; i < resultadosPropios.getSize(); i++) {
            ...
            listaResultados.add("🏠 " + inm.getCodigo() + " en " + inm.getBarrioZona() + " | Precio: $"
                    + String.format("%,.0f", inm.getPrecio()));
        }
    ...
    public String generarRecomendaciones(@RequestParam String idCliente, RedirectAttributes redirectAttrs) {
        ...
        if (recos.getSize() == 0) {
            ...
            for (int i = 0; i < recos.getSize(); i++) {
                ...
                listaSugerencias.add("📌 " + inm.getCodigo() + " - " + inm.getBarrioZona() + " | Precio: $"
                        + String.format("%,.0f", inm.getPrecio()));
            }
    ...
    public String generarReportes(RedirectAttributes redirectAttrs) {
        ...
        for (int i = 0; i < rankingAsesores.getSize(); i++) {
            ...
            listaAsesores.add("🏆 " + a.getNombre() + " | Ventas/Cierres: " + a.getNumeroCierres());
        }
    ...
    public String panelGerencial(@RequestParam(defaultValue = "Norte") String zonaSimulacion,
            RedirectAttributes redirectAttrs) {
        ...
        for (int i = 0; i < vips.getSize(); i++) {
            ...
            listaVips
                    .add("🌟 " + c.getNombre() + " (Presupuesto: $" + String.format("%,.0f", c.getPresupuesto()) + ")");
        }
        ...
        redirectAttrs.addFlashAttribute("listaVips",
                listaVips.isEmpty() ? List.of("No hay clientes VIP detectados.") : listaVips);
        redirectAttrs.addFlashAttribute("listaRankingZonas",
                listaRanking.isEmpty() ? List.of("No hay actividad.") : listaRanking);
    ...
    /**
     * Convierte el Árbol Binario en un JSON recursivo compatible con la librería
     * gráfica GoJS.
     */
    ...
    private void cargarDatosPrueba() {
        ...
        Apartamento apt1 = new Apartamento("APT-001", "Calle 10N", "Armenia", "Norte", "Venta", 250000000.0, 65.0, 3, 2,
                "Nuevo", true, asesor1, true, 200000.0);
        ...
        LocalComercial loc1 = new LocalComercial("LOC-001", "Carrera 14", "Armenia", "Centro", "Venta", 500000000.0,
                120.0, 1, 2, "Remodelado", true, asesor2, true, "Comercial Mixto");
        ...
        Cliente cliente1 = new Cliente("C-001", "Andrés", "andres@gmail.com", "333", "Comprador", 300000000.0,
                "Apartamento", 2);
        Cliente cliente2 = new Cliente("C-002", "Nathaly", "nat@gmail.com", "444", "Inversionista", 600000000.0,
                "LocalComercial", 0);
